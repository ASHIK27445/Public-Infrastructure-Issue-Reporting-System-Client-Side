    ...
    <Shell>
      ...
      <div className="max-w-5xl mx-auto px-4 py-5 flex gap-6 items-start">
        ...
        <div className="w-64 shrink-0 sticky top-24 space-y-3">
          ...
          {freeTotal > 0 && (
            <SidebarStatRow label="Free Participants" attended={statsData?.stats?.freeParticipantsAttended || 0} total={freeTotal} color="emerald" />
          )}
          <div className="border-t border-zinc-700 pt-3 mt-3">
            <SidebarStatRow label="Waitlisted" total={statsData?.stats?.waitlisted || 0} color="amber" noProgress />
          </div>
        </div>
      </div>
...
function SidebarStatRow({ label, attended, total, color, noProgress }) {
  const colors = {
    blue:    { bar: "bg-blue-500",   text: "text-blue-400",   badge: "bg-blue-500/10" },
    violet:  { bar: "bg-violet-500", text: "text-violet-400", badge: "bg-violet-500/10" },
    emerald: { bar: "bg-emerald-500",text: "text-emerald-400",badge: "bg-emerald-500/10" },
    amber:   { bar: "bg-amber-500",  text: "text-amber-400",  badge: "bg-amber-500/10" },
  };
  const c = colors[color];
  const pct = total > 0 ? Math.round((attended / total) * 100) : 0;

  return (
    <div className={`rounded-xl border border-zinc-700 p-3 ${c.badge}`}>
      ...
      {!noProgress && (
        <>
          ...
          <p className="text-[10px] text-gray-500 mt-1">{pct}% checked in</p>
        </>
      )}
    </div>
  );
